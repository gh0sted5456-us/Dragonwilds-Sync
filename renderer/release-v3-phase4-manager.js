(() => {
  ...
  function editorMarkup(row={}, preview='') {
    const isNew=!row.id; const image=preview || '';
    return `<div class="v3p4-badge-editor" data-badge-id="${esc(row.id||'')}">
      ...
      <div class="v3p4-editor-actions"><button class="btn primary" data-v3p4-badge-save>${isNew?'Add':'Save'}</button>${isNew?'':`<button class="btn ghost" data-v3p4-badge-up>Move Up</button><button class="btn ghost" data-v3p4-badge-down>Move Down</button><button class="btn danger" data-v3p4-badge-remove>Remove</button>`}</div>
  ...
  async function loadManager(profileId) {
    const result = await api.invoke('v3.phase4.badges.list',{id:profileId}); const rows=Array.isArray(result?.badges)?result.badges:[];
    let host=managers.get(profileId); if(host?.isConnected){host.classList.remove('hidden');return;}
  ...
  function selectBadge(host,id) {
    const row=(host._rows||[]).find(x=>x.id===id)||{}; host.querySelectorAll('.v3p4-manager-item').forEach(x=>x.classList.toggle('active',x.dataset.v3p4BadgeId===id)); host.querySelector('[data-v3p4-editor]').innerHTML=editorMarkup(row);
  }

  async function refreshManager(host, preferred='') {
    const id=host.querySelector('[data-v3p4-manager]')?.dataset.v3p4Manager || host.querySelector('.v3p4-badge-manager')?.dataset.v3p4Manager; const result=await api.invoke('v3.phase4.badges.list',{id}); host._rows=Array.isArray(result?.badges)?result.badges:[];
    const list=host.querySelector('.v3p4-manager-list'); list.innerHTML=host._rows.map(row=>`<button class="v3p4-manager-item" data-v3p4-badge-id="${esc(row.id)}"><span>${esc(row.name||row.label)}</span><small>${row.enabled===false?'Disabled':'Enabled'}</small></button>`).join(''); selectBadge(host, preferred || host._rows[0]?.id || '');
  }
  ...
  document.addEventListener('click', async (event) => {
    ...
    if(event.target.closest('[data-v3p4-badge-save]')){try{await saveEditor(host);}catch(e){host.querySelector('[data-v3p4-manager-status]').textContent=String(e?.message||e);}return;}
