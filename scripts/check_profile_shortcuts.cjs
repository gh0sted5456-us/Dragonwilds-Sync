...
try {
  const gui = path.join(shortcutFixture, 'Dragonwilds Sync and Launcher-Portable-3.1.0.exe');
  const headless = path.join(shortcutFixture, 'Dragonwilds Sync Headless-3.1.0.exe');
  ...
  assert.strictEqual(resolveHeadlessShortcutTarget({ executablePath: gui, version: '3.1.0' }), path.resolve(headless));
