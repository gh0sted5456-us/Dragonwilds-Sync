...
const protocol = need('backend/runtime_worker_protocol.py', [
  'PROTOCOL_VERSION = 1','MAX_MESSAGE_BYTES','AF_PIPE','AF_UNIX','send_bytes','recv_bytes','DWSYNC_RUNTIME_WORKER_AUTH','atomic_json','0o600'
]);
const worker = need('backend/runtime_worker.py', [
  'WORKER_READY','PING','GET_STATUS','STOP','PROTOCOL_MISMATCH','COMMAND_NOT_ALLOWED','worker-state.json','gamePid": None','authRef'
]);
const supervisor = need('backend/worker_supervisor.py', [
  'WorkerSupervisor','--runtime-worker','sys.executable','token_urlsafe','dws-secret','CREATE_NEW_PROCESS_GROUP','start_new_session','reconcile','cleanup_stale','spawn','stop','list_status'
]);
...
for (const forbidden of ['server_engine','runtime_manager','network_service','DirectoryNetworkService','SyncShareServer','SteamCMD']) {
  if (worker.includes(forbidden)) failures.push(`Phase 2 worker must not migrate runtime logic yet: ${forbidden}`);
}
...
if (!supervisor.includes('SecretStore')) failures.push('supervisor must use encrypted secret-reference authority for reconnect credentials');
...
console.log('[Runtime Worker Phase 2] PASS · same-executable headless mode, authenticated local IPC, supervisor/reattach, state/log boundaries and no game migration verified');
