...
def test_public_catalog_remote_login_audit_and_structured_action():
    with tempfile.TemporaryDirectory() as td:
        ...
        controller.set_remote_admin_callbacks(
            authenticate=lambda name, username, password: {"ok": name == "Ashen Home" and not username and password == "admin-secret", "world_id": "profile-1", "world_name": "Ashen Home", "username": "owner", "role": "owner", "permissions": {**directory_host.REMOTE_PERMISSION_DEFAULTS, "write_config": False, "use_console": True, "send_announcements": True}},
        ...
        try:
            ...
            assert b'data-tab="items"' not in portal and b"Item Spawner" not in portal
            assert b'data-tab="announcements"' in portal and b"Broadcast Messages" in portal
            assert b"spawner_catalog" not in portal and b"spawner_item" not in portal and b"announcement_send" in portal
            ...
            assert b'Item Builder' not in portal and b'dws-item-categories' not in portal
