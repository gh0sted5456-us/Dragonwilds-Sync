...
try {
  const gui = path.join(shortcutFixture, 'Dragonwilds Sync and Launcher-Portable-3.0.3.exe');
  const headless = path.join(shortcutFixture, 'Dragonwilds Sync Headless-3.0.3.exe');
  ...
  assert.strictEqual(resolveHeadlessShortcutTarget({ executablePath: gui, version: '3.0.3' }), path.resolve(headless));
