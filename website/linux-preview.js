(() => {
  ...
  function render(host) {
    ...
    host.innerHTML = `<section class="linux-detection" aria-labelledby="linux-detection-title">
      ...
      <div class="linux-samples"><span>Try a detected host</span>${data.examples.map((item,index)=>`<button type="button" class="${index===state.selected?'is-active':''}" data-linux-sample="${index}"><img src="assets/distros/${esc(item.distro_icon||'linux')}.svg" alt="">${esc(item.short_name||item.distro_name)}</button>`).join('')}</div>
