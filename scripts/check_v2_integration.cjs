...
must(css.includes('world-grid{align-items:stretch') && css.includes('grid-auto-rows:1fr'), 'Placards are not normalized through the renderer layout');
...
must(appV2.includes("syncTabLabels={settings:'Hosting',manifest:'Directory Sources',remote:'Remote Access'}") &&
  appV2.includes("syncTab('settings')") && appV2.includes("syncTab('manifest')") &&
  appV2.includes("syncTab('remote')") && !appV2.includes("syncTab('live')"),
  'Hosting must expose configuration, directory sources, and Remote Access without an embedded preview');
