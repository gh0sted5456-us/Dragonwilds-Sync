...
function publicWorld(row: Record<string, unknown>, offlineAfterSeconds: number) {
  ...
  return json(
    {
      ...
      worlds: results.map((row) => publicWorld(row, offlineAfter)),
    },
    200,
...
async function getWorld(env: Env, worldId: string): Promise<Response> {
  const offlineAfter = clampInt(env.OFFLINE_AFTER_SECONDS, 1800, 60, 86400);
  const row = await env.DB.prepare("SELECT * FROM worlds WHERE world_id = ? AND is_listed = 1")
    .bind(worldId)
    .first<Record<string, unknown>>();

  if (!row) return json({ error: "not_found" }, 404, publicCorsHeaders());
  return json(publicWorld(row, offlineAfter), 200, {
    ...publicCorsHeaders(),
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      return json({ ok: true, service: "dragonwilds-sync-directory" }, 200, publicCorsHeaders());
    }
