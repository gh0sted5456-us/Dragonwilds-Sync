...
def png_dimensions(payload: bytes) -> tuple[int, int]:
    ...
    if bytes(payload[12:16]) != b"IHDR":
        raise ValueError("PNG badge is missing its IHDR header.")
    width, height = struct.unpack(">II", bytes(payload[16:24]))
    if width <= 0 or height <= 0:
        raise ValueError("PNG badge dimensions are invalid.")
...
def decode_png_data(value: object) -> bytes:
    ...
    try:
        ...
        raise ValueError("Custom badge icon is not valid base64 PNG data.") from exc
    if len(payload) > MAX_BADGE_BYTES:
        raise ValueError(f"Custom badge PNG exceeds the {MAX_BADGE_BYTES // 1024} KiB limit.")
...
def badge_asset_bytes(name: object) -> bytes:
    ...
    if not _HASH_RE.fullmatch(key):
        return b""
    target = BADGE_CACHE_DIR / f"{key}.png"
    try:
        payload = target.read_bytes()
        png_dimensions(payload)
        if len(payload) <= MAX_BADGE_BYTES and hashlib.sha256(payload).hexdigest() == key:
            return payload
...
def _normalize_badge(row: dict, *, existing: dict | None = None) -> dict:
    ...
    if not name:
        raise ValueError("Custom badge name is required.")
    tooltip = _clean_text(row.get("tooltip") if "tooltip" in row else existing.get("tooltip"), 240) or name
    badge_id = _clean_text(row.get("id") or existing.get("id"), 64).casefold()
    badge_id = re.sub(r"[^a-z0-9._-]+", "-", badge_id).strip("-")
    if not badge_id or not _ID_RE.fullmatch(badge_id):
        badge_id = "badge-" + secrets.token_hex(8)
    asset_hash = str(row.get("asset_hash") or existing.get("asset_hash") or "").casefold()
    asset_path = str(row.get("asset_path") or existing.get("asset_path") or "")[:160]
    image_data = row.get("image_data") or row.get("png_data") or row.get("data_url")
    if image_data:
        cached = cache_badge_png(image_data)
        asset_hash, asset_path = cached["asset_hash"], cached["asset_path"]
    if asset_hash and not _HASH_RE.fullmatch(asset_hash):
        raise ValueError("Custom badge asset hash is invalid.")
    if asset_hash and not asset_path:
        asset_path = f"/assets/placards/badge-{asset_hash}.png"
    return {
        "id": badge_id,
        "name": name,
        "label": name,
...
def _profile(profile_id: str) -> dict:
    ...
    if not profile:
        raise ValueError("Custom badge management currently requires an existing local hosted World profile.")
    return profile


def list_badges(profile_id: str, *, include_disabled: bool = True) -> list[dict]:
    rows = (_profile(profile_id).get("presentation") or {}).get("custom_badges")
    if not isinstance(rows, list):
        rows = _profile(profile_id).get("custom_badges")
    result = []
    for raw in rows or []:
...
def save_badges(profile_id: str, rows: list[dict]) -> list[dict]:
    if not isinstance(rows, list):
        raise ValueError("Custom badges must be a list.")
    result, seen = [], set()
    for raw in rows[:MAX_BADGES_PER_WORLD]:
        if not isinstance(raw, dict):
            continue
        row = _normalize_badge(raw)
        key = row["id"].casefold()
        if key in seen:
            raise ValueError(f"Duplicate custom badge ID: {row['id']}")
        seen.add(key); result.append(row)
    profile = _profile(profile_id)
    presentation = profile.setdefault("presentation", {})
    presentation["custom_badges"] = result
    # Keep one compatibility mirror while older renderers still read this key.
    profile["custom_badges"] = result
...
def add_badge(profile_id: str, row: dict) -> list[dict]:
    ...
    if len(rows) >= MAX_BADGES_PER_WORLD:
        raise ValueError(f"A World may publish at most {MAX_BADGES_PER_WORLD} custom badges.")
    rows.append(_normalize_badge(row if isinstance(row, dict) else {}))
    return save_badges(profile_id, rows)


def update_badge(profile_id: str, badge_id: str, patch: dict) -> list[dict]:
    rows = list_badges(profile_id)
    wanted = str(badge_id or "").casefold(); found = False
...
def remove_badge(profile_id: str, badge_id: str) -> list[dict]:
    wanted = str(badge_id or "").casefold()
    rows = [row for row in list_badges(profile_id) if str(row.get("id") or "").casefold() != wanted]
    return save_badges(profile_id, rows)
...
def reorder_badges(profile_id: str, ordered_ids: list[str]) -> list[dict]:
    ...
    return save_badges(profile_id, result)
...
def public_badge_refs(profile_id: str) -> list[dict]:
    ...
    for row in list_badges(profile_id, include_disabled=False):
        result.append({
            "id": row["id"], "label": row["name"], "tooltip": row["tooltip"],
            "asset_hash": row.get("asset_hash") or "", "asset_url": row.get("asset_path") or "",
