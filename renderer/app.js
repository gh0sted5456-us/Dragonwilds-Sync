(() => {
  ...
  const DRAGON_CORE_GROUPS = {
    Magic:['Rune','Catalyst','MagicComponent','Essence'], Ammunition:['Ammo','Arrow','Bolt','Throwable'], Currency:['Currency','Token'],
    ...
    FarmingFishing:['Seed','Sapling','FarmingMaterial','Compost','Fertilizer','Fish','Bait'], CraftingBuilding:['Plan','Recipe','Decoration','Furniture','BuildingMaterial'],
    Protected:['Quest','Key','Artifact','Mount','Pet']
  };
  const defaultDragonCoreValue=(group,name,kind)=>kind==='stack'?(group==='Protected'?1:(group==='Currency'?99999:(['Magic','Ammunition'].includes(group)?9999:300))):(group==='Protected'||(group==='Ammunition'&&['Ammo','Arrow','Bolt'].includes(name))?-1:0);
  ...
  async function refreshSinglePlayerInventory(quiet = false, rescan = false) {
    try {
      const response = await api.invoke('singleplayer.inventory', { profile_id: state.data?.client?.active_private_world_id || singleplayerWorld()?.id, rescan });
  ...
  function recommendedModsMarkup(target='client') {
    ...
    return `<section class="recommended-mods-panel"><div class="panel-header"><div><h3>Creator Recommended Mods</h3><span class="panel-subtitle">GitHub-fed links and metadata only. Downloads remain with the original author and hosting platform.</span></div><button class="btn ghost compact-btn" data-recommended-refresh>Refresh List</button></div>${rows.length?`<div class="recommended-mod-grid">${rows.map((mod)=>`<article class="recommended-mod-card"><div><strong>${escapeHtml(mod.name||'Mod')}</strong><small>${escapeHtml(mod.author||'Community author')} · ${escapeHtml(String(mod.mod_type||'mod').toUpperCase())} · ${escapeHtml(mod.change||'quality of life')}</small><div class="mod-tag-row">${(mod.targets||[]).map((value)=>`<span class="mod-tag">${escapeHtml(String(value).toUpperCase())}</span>`).join('')}</div></div><button class="btn ghost compact-btn" data-recommended-open="${escapeHtml(mod.page_url||'')}">View Source</button></article>`).join('')}</div>`:'<div class="empty-state">No recommendations are available for this target yet.</div>'}</section>`;
  }
  ...
  function renderSinglePlayerDetail(world) {
    ...
    if(tab==='overview'){
      ...
      body=`<div class="server-tab-body">${dragonCoreEditorMarkup(world)}</div>`;
  ...
  function managedWorldConfigMarkup(configs, isActive) {
    ...
    return `<details class="panel collapsible-panel" open style="margin-top:18px"><summary class="panel-header"><div><h2>World Config & Mod Files</h2><span class="panel-subtitle">Grouped by origin · JSON, Lua, INI, CFG and TXT through the bundled Monaco editor.</span></div><div class="header-actions"><button class="btn ghost" id="refresh-world-maintenance">Refresh</button>${isActive ? '<button class="btn ghost" id="regenerate-mods-txt">Regenerate mods.txt</button>' : ''}</div></summary><div class="panel-body">${rows ? `<div class="config-file-list">${rows}</div>` : `<div class="empty-state">${isActive ? 'No editable config/Lua/JSON files were found in the World config and mod roots.' : 'Activate this World to inspect its live files.'}</div>`}<div class="identity-box"><strong>Managed file policy</strong><p>Dragonwilds Sync unlocks a managed file only for an atomic write, validates JSON, and returns it to read-only. Hotload and client-sync policies remain explicit; credential-like files never synchronize to clients.</p></div></div></details>`;
  }

  function dragonCoreEditorMarkup(world) {
    const cfg=world.dragon_core||{},categories=cfg.categories||{};
    const groups=Object.entries(DRAGON_CORE_GROUPS).map(([group,names])=>`<details class="panel collapsible-panel dragon-core-group" open><summary class="panel-header"><div><h2>${escapeHtml(group.replace(/([a-z])([A-Z])/g,'$1 / $2'))}</h2><span class="panel-subtitle">DragonCore category rules · vanilla and modded ItemData</span></div><span>${names.length} rules</span></summary><div class="panel-body"><div class="dragon-core-grid"><div class="dragon-core-head"><b>Item category</b><b>Stack size</b><b>Weight</b></div>${names.map((name)=>{const row=categories[group]?.[name]||{};return `<label class="dragon-core-row"><strong>${escapeHtml(name.replace(/([a-z])([A-Z])/g,'$1 $2'))}</strong><input class="field" type="number" min="1" max="999999" step="1" data-dragon-core-stack data-group="${escapeHtml(group)}" data-name="${escapeHtml(name)}" value="${escapeHtml(row.stack??defaultDragonCoreValue(group,name,'stack'))}"/><input class="field" type="number" min="-1" max="100000" step="0.01" data-dragon-core-weight data-group="${escapeHtml(group)}" data-name="${escapeHtml(name)}" value="${escapeHtml(row.weight??defaultDragonCoreValue(group,name,'weight'))}"/></label>`;}).join('')}</div></div></details>`).join('');
    return `<div class="dragon-core-editor"><section class="spawner-intro"><div><div class="eyebrow">DragonCore · Profile Configuration</div><h2>Item Editor</h2><p>These fields write this World profile’s DragonCore stack and weight configuration. The functional mod stays hidden and changes apply when this profile is active.</p></div><div class="spawner-status-row"><span class="status-pill online">PROFILE OWNED</span><span class="status-pill unknown">DEBUG OFF</span></div></section><div class="settings-row"><div class="settings-copy"><strong>DragonCore features</strong><span>Player mode is used for Singleplayer/Co-Op; Server mode is generated for dedicated Worlds.</span></div><div class="toggle-list"><label><input type="checkbox" id="dragon-core-enabled" ${cfg.enabled===false?'':'checked'}/> Enabled</label><label><input type="checkbox" id="dragon-core-stacks" ${cfg.stacks_enabled===false?'':'checked'}/> Stack rules</label><label><input type="checkbox" id="dragon-core-weights" ${cfg.weights_enabled===false?'':'checked'}/> Weight rules</label></div></div>${groups}<div class="sticky-save-row"><button class="btn primary" id="save-dragon-core">Save Item Rules</button><small>Weight −1 preserves the game/mod authored value; 0 makes an item weightless.</small></div></div>`;
  }
  ...
  function renderServerDetail(world) {
    ...
    if (tab === 'overview') {
      ...
      body = `<div class="items-enemies-page"><div class="items-enemies-tabs"><button class="${state.itemsEnemiesTab==='editor'?'active':''}" data-items-enemies-tab="editor">Item Editor</button><button class="${state.itemsEnemiesTab==='spawner'?'active':''}" data-items-enemies-tab="spawner">Spawner</button></div>${state.itemsEnemiesTab==='editor'?dragonCoreEditorMarkup(p):body}</div>`;
    } else if (tab === 'console') {
  ...
  async function rollbackNexusUnit(unit, target='singleplayer') {
    ...
    try{
      ...
      }else if(standaloneRemote){
        content=webhostTabs+`
          <section class="settings-section"><div class="panel-header" style="padding:0 0 10px"><div><h2 style="margin:0">Remote Server Access</h2><div style="font-size:9px;color:var(--muted);margin-top:4px">Manage the listener, desktop-owned accounts, and the exact permissions granted to remote sessions.</div></div><span class="status-pill ${directoryHostStatus.serving&&remoteSurface?'online':'unknown'}">${directoryHostStatus.serving&&remoteSurface?'ONLINE':'OFFLINE'}</span></div>
            <div class="settings-row"><div class="settings-copy"><strong>Website listener</strong><span>Remote login uses the shared WebHost TCP listener. This switch starts or stops that listener without changing any hosted Dragonwilds process.</span></div><button class="toggle ${directoryHost.enabled?'on':''}" id="toggle-directory-host"></button></div>
            <div class="settings-row"><div class="settings-copy"><strong>Remote Server login</strong><span>Enables authenticated, World-scoped remote administration. Public directory publishing remains independently controlled.</span></div><button class="toggle ${remoteAdmin.enabled!==false?'on':''}" id="toggle-webhost-remote-admin" aria-label="Remote Server login" aria-pressed="${remoteAdmin.enabled!==false?'true':'false'}"></button></div>
            <div class="settings-row"><div class="settings-copy"><strong>Listener address</strong><span>Set the TCP port and optional public HTTPS/DNS address used by administrators.</span></div><div class="health-evidence-grid"><label><small>Website TCP port</small><input class="field" id="directory-host-port" type="number" min="1" max="65535" value="${escapeHtml(directoryHost.port||27080)}"/></label><label><small>Public website / DNS URL</small><input class="field" id="directory-host-public-url" value="${escapeHtml(directoryHost.public_base_url||'')}" placeholder="https://worlds.example.com"/></label></div></div>
            <input type="hidden" id="directory-host-publication-mode" value="${escapeHtml(directoryHost.publication_mode||'manual')}"/><input type="hidden" id="directory-host-identity" value="${escapeHtml(webhostIdentity)}"/>
          </section>
          <section class="settings-section webhost-authority"><div class="panel-header"><div><h2>Default permissions</h2><span class="panel-subtitle">These are the portal defaults. Each user keeps an individual permission snapshot that can be edited below.</span></div><span class="status-pill unknown">DENY WRITE BY DEFAULT</span></div><div class="webhost-permission-grid">
            ...
            ${permissionChoice('write_maintenance','Write maintenance','Change the World maintenance schedule remotely.',false)}
            ${permissionChoice('view_mods','View mods','Read the hosted World mod inventory.',true)}
            ${permissionChoice('write_mods','Write mod metadata','Change tags and hotload metadata through audited calls.',false)}
            ${permissionChoice('view_config','View configs','List and open launcher-managed World configuration files.',true)}
            ${permissionChoice('write_config','Write live configs','Save managed config and mod text through the desktop file boundary.',false)}
            ${permissionChoice('view_spawner','View item spawner','Browse the server item catalog and connected-player targets.',true)}
            ${permissionChoice('use_spawner','Use item spawner','Give bounded item stacks through the Admin Tools route.',false)}
            ${permissionChoice('view_console','View game console','Browse declared RSDW Toolkit commands and audited history.',true)}
            ${permissionChoice('use_console','Use game console','Run confirmed, declared commands through the bounded bridge.',false)}
            ${permissionChoice('view_audit','View audit','See remote logins, denied commands, and completed actions.',true)}
            ${permissionChoice('send_announcements','Send announcements','Publish color-coded passive launcher announcements.',false)}
          ...
          <section class="settings-section webhost-authority"><div class="panel-header"><div><h2>Server users</h2><span class="panel-subtitle">Create accounts locally, assign each account to a hosted World, and edit its individual permissions.</span></div><button class="btn primary" id="add-webhost-user">Add Server User</button></div><div class="config-file-list">${remoteUsers.length?remoteUsers.map((user)=>`<div class="config-file-row"><div><strong>${escapeHtml(user.username||'User')}</strong><small>${escapeHtml((state.data?.server_profiles||[]).find((profile)=>String(profile.id)===String(user.world_id))?.name||user.world_id||'No World')} · ${Object.values(user.permissions||{}).filter(Boolean).length} granted · ${user.enabled===false?'Disabled':'Enabled'}</small></div><div class="header-actions"><button class="btn ghost" data-edit-webhost-user="${escapeHtml(user.username||'')}">Edit Permissions</button><button class="btn danger" data-delete-webhost-user="${escapeHtml(user.username||'')}">Delete</button></div></div>`).join(''):'<div class="empty-state compact-empty">No server users yet. Select Add Server User to create the first account.</div>'}</div></section>
          <section class="settings-section webhost-authority"><div class="panel-header"><div><h2>Permission requests</h2><span class="panel-subtitle">Remote categories remain unavailable until approved here.</span></div><span class="status-pill ${pendingPermissionRequests.length?'unknown':'online'}">${pendingPermissionRequests.length} PENDING</span></div><div class="config-file-list">${pendingPermissionRequests.length?pendingPermissionRequests.map((request)=>`<div class="config-file-row"><div><strong>${escapeHtml(request.username||'User')} requests ${escapeHtml(String(request.permission||'').replaceAll('_',' '))}</strong><small>${new Date(Number(request.requested_at||0)*1000).toLocaleString()}</small></div><div class="header-actions"><button class="btn primary" data-resolve-webhost-request="${escapeHtml(request.id||'')}" data-approve="1">Approve</button><button class="btn ghost" data-resolve-webhost-request="${escapeHtml(request.id||'')}" data-approve="0">Deny</button></div></div>`).join(''):'<div class="empty-state compact-empty">No remote permission requests.</div>'}</div></section>
          <section class="settings-section"><div class="settings-row"><div class="settings-copy"><strong>Apply remote access settings</strong><span>Persists the listener, account defaults, and permission changes. Existing user-specific permissions remain intact until edited.</span></div><div class="header-actions"><button class="btn primary" id="save-directory-host">Save / Apply</button>${hostedBase?`<button class="btn ghost" data-open-external="${escapeHtml(hostedBase+'/admin/login')}">Open Remote Login ↗</button>`:''}</div></div></section>`;
      }else{
  ...
  function renderSettings() {
    ...
    const standaloneRemote = routedRemote || (routedWebhost && state.webhostTab === 'remote') || (externalSettings && externalSettingsTab === 'remote');
    ...
    if (tab === 'mods') {
      ...
      <section class="settings-section"><div class="panel-header"><div><h2>Creator &amp; Community Recommendations</h2><span class="panel-subtitle">The creator feed comes from GitHub. Communities can publish the same JSON format and share its URL without packaging any mod archive.</span></div><button class="btn primary" data-recommended-refresh>Refresh Lists</button></div>
        ...
        <div class="recommended-mod-grid">${recommendationRows.slice(0,20).map((mod)=>`<article class="recommended-mod-card"><div><strong>${escapeHtml(mod.name||'Mod')}</strong><small>${escapeHtml(mod.author||mod.source_name||'Community')} · ${escapeHtml(String(mod.mod_type||'mod').toUpperCase())}</small><div class="mod-tag-row">${(mod.targets||[]).map((value)=>`<span class="mod-tag">${escapeHtml(String(value).toUpperCase())}</span>`).join('')}</div></div><button class="btn ghost compact-btn" data-recommended-open="${escapeHtml(mod.page_url||'')}">Open Source</button></article>`).join('')||'<div class="empty-state">Refresh the GitHub feed to load recommendations.</div>'}</div>
...
`;
  ...
  function renderWelcome() {
    ...
    return `${renderTitlebar()}<div class="fantasy-entry"><div class="fantasy-entry-art" style="background-image:url('${escapeHtml(state.data?.application?.loading_art_url||'assets/theme/animated-splash.gif')}')"></div><div class="fantasy-entry-shade"></div><div class="fantasy-entry-card"><img src="assets/application-icon.png" alt="Dragonwilds Sync" /><div class="eyebrow">${escapeHtml(window.DWSYNC_RELEASE_META?.name || 'V2')}</div><h1>Worlds, Mods, and Servers — One Launcher</h1><p>Everything is loaded. Enter when you're ready.</p>${updateCard}${changelogCard}<div class="entry-tip"><strong>Field Note</strong><span>${escapeHtml(tip)}</span></div><button class="btn primary entry-button" id="enter-launcher">ENTER</button><small>Dragonwilds Sync V2 · RSDWL v3 · Smart Updates</small></div></div>`;
  }
  ...
  function bindEvents() {
    ...
    root.querySelectorAll('[data-items-enemies-tab]').forEach((button)=>button.addEventListener('click',()=>{state.itemsEnemiesTab=button.dataset.itemsEnemiesTab==='spawner'?'spawner':'editor';render();}));
    root.querySelector('#save-dragon-core')?.addEventListener('click',async()=>{
      const categories={};
      root.querySelectorAll('[data-dragon-core-stack]').forEach((input)=>{const group=input.dataset.group,name=input.dataset.name;if(!group||!name)return;(categories[group]||(categories[group]={}))[name]={...(categories[group]?.[name]||{}),stack:Math.max(1,Math.min(999999,Math.round(Number(input.value||1))))};});
      root.querySelectorAll('[data-dragon-core-weight]').forEach((input)=>{const group=input.dataset.group,name=input.dataset.name;if(!group||!name)return;(categories[group]||(categories[group]={}))[name]={...(categories[group]?.[name]||{}),weight:Math.max(-1,Math.min(100000,Number(input.value||0)))};});
      const dragon_core={enabled:!!root.querySelector('#dragon-core-enabled')?.checked,stacks_enabled:!!root.querySelector('#dragon-core-stacks')?.checked,weights_enabled:!!root.querySelector('#dragon-core-weights')?.checked,categories};
      const privateWorld=state.route==='world-detail'&&activeWorld()?.kind==='singleplayer'?(privateWorldById(state.selectedWorldId)||singleplayerWorld()):null;
      try{
        if(privateWorld){const response=await api.invoke('singleplayer.profile.update',{profile_id:privateWorld.id,profile:{dragon_core}});if(response.state)setData(response.state);}
        else {const world=activeServerWorld();if(!world)return;const response=await api.invoke('server.world.update',{id:world.id,dragon_core});setData(response);}
        render();toast('DragonCore item rules saved','The hidden functional mod was regenerated for this World profile.','success');
      }catch(error){toast('Could not save DragonCore rules',error.message,'error');}
    });
  ...
  async function executeLauncherCommand(rawCommand) {
    ...
    if(command==='start server'){
      ...
      const response=await api.invoke('server.world.broadcast',{id:world.id});if(!response.result?.running)throw new Error('The dedicated process did not report a running state.');if(response.state)setData(response.state);render();toast('Server started',world.name||'Dedicated World','success');return;
    }
