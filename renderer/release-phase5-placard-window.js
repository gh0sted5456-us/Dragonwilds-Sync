(() => {
  ...
  function openPlacard(id, source = null) {
    ...
    win.innerHTML = `<div class="phase5-placard-titlebar"><strong>${esc(title)}</strong><div><button class="phase5-placard-window-control" data-phase5-placard-min title="Minimize" aria-label="Minimize">—</button><button class="phase5-placard-window-control" data-phase5-placard-max title="Maximize / Restore" aria-label="Maximize or restore">□</button><button class="phase5-placard-window-control close" data-phase5-placard-close title="Close" aria-label="Close">×</button></div></div><div class="phase5-placard-window-body"></div>`;
    const body=win.querySelector('.phase5-placard-window-body');
    const snapshot=source?.cloneNode?.(true);
    if(snapshot){snapshot.classList.add('phase5-placard-snapshot');snapshot.removeAttribute('data-world-id');snapshot.removeAttribute('tabindex');snapshot.querySelectorAll('[id]').forEach((node)=>node.removeAttribute('id'));snapshot.querySelectorAll('button,a,input,select,textarea').forEach((node)=>node.setAttribute('tabindex','-1'));body.appendChild(snapshot);}
    else body.innerHTML=`<article class="world-card phase5-placard-snapshot"><div class="v3p4-window-summary"><div class="eyebrow">World Placard</div><h2>${esc(title)}</h2><p>Live World identity, joining details, compatibility and heartbeat status.</p></div></article>`;
