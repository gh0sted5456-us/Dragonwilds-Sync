...
def _png_hash_from_data_url(value: object) -> str:
    try: payload = decode_png_data(value)
...
def normalize_custom_badges(values: object) -> list[dict[str, str]]:
    ...
    for raw in values:
        if isinstance(raw, str):
            ...
            if digest and not asset_url: asset_url = f"/assets/placards/badge-{digest}.png"
            identity_seed = _text(raw.get("id"), 64) or digest or f"{label}|{tooltip}"
            badge_id = re.sub(r"[^a-z0-9._-]+", "-", identity_seed.casefold()).strip("-")[:64]
            if not badge_id: badge_id = "badge-" + hashlib.sha256(identity_seed.encode("utf-8")).hexdigest()[:16]
            row = {"id": badge_id, "label": label, "tooltip": tooltip,
...
def decorate_public_snapshot(snapshot: dict, raw: dict) -> dict:
    ...
    if not isinstance(badge_source, list): badge_source = presentation.get("custom_badges")
    if not isinstance(badge_source, list): badge_source = raw.get("badges")
    ...
    for badge in badge_refs:
        if badge["label"].casefold() not in {x.casefold() for x in legacy_labels}: legacy_labels.append(badge["label"])
    result["badges"] = legacy_labels[:32]
    if badge_refs: result["badge_refs"] = badge_refs
    else: result.pop("badge_refs", None)
    compatibility = raw.get("platforms")
    if compatibility is None: compatibility = raw.get("platform_compatibility") or presentation.get("platform_compatibility")
    if compatibility is None and isinstance(raw.get("compatibility"), dict): compatibility = raw["compatibility"].get("platforms")
    platforms = normalize_platforms(compatibility)
    if platforms:
        result["platforms"] = platforms
        result["platform_refs"] = [{"id": row["id"], "displayName": row["displayName"],
            "directSupportUrl": row.get("directSupportUrl") or "", "fallbackInfoUrl": row.get("fallbackInfoUrl") or "",
            "verified": bool(row.get("verified"))} for row in platforms_for(platforms)]
    else:
...
def _enrich_raw_from_profile(profile_id: str, kind: str, raw: dict) -> dict:
    ...
    for key in ("tags", "platform_compatibility", "community_rules", "description", "custom_badges"):
        ...
        if value not in (None, "", []): merged[key] = deepcopy(value)
    if isinstance(presentation.get("custom_badges"), list): merged["custom_badges"] = deepcopy(presentation["custom_badges"])
    if isinstance(presentation.get("platform_compatibility"), dict): merged.setdefault("platform_compatibility", deepcopy(presentation["platform_compatibility"]))
    return merged
...
def heartbeat_status(network: Any, profile_id: str, kind: str = "dedicated") -> dict:
    ...
    if not profile_id: return {"state": "Disabled", "active": False, "destinations": [], "last_success_at": None}
    world = network.world_status(profile_id, kind); service = network.status()
    active = str((service.get("active_world") or {}).get("profile_id") or "") == profile_id
    configured: list[dict] = []
    if bool(world.get("public_directory_enabled")): configured.append({"id": "official", "name": "Dragonwilds Sync Network", "enabled": True})
    ...
    if not configured or not active: return {"state": "Disabled", "active": active, "destinations": configured, "last_success_at": None}
    try: delivery = network._delivery_state().get("destinations") or {}
    except Exception: delivery = {}
    outcomes: list[dict] = []; attempts = 0; successes: list[float] = []
    for row in configured:
        current = delivery.get(row["id"]) or {}; attempted = bool(current.get("last_attempt_at"))
        ...
        outcomes.append({**row, "ok": ok, "last_attempt_at": current.get("last_attempt_at"), "last_success_at": success_at,
