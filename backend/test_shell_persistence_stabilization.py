...
def _settings_persistence() -> None:
    ...
    assert disk["mods"]["inventory_count"] == 5
    inventory_names = {row.get("name") for row in disk["mods"]["inventory"]}
    assert inventory_names == {"ActualLua", "DragonCore", "DragonLink", "SchemaPack", "ActualPak"}
