...
- **`CAPABILITIES.md`** — detailed subsystem capability matrix, data ownership, protocols, and operational boundaries.
...
- **Live Map & Tracking** — shared Ashenfall map with optional live positions from the launcher-managed, hidden RSDWTools baseline bridge. Bridge debug output is disabled by default, and tracking markers are withheld until map calibration is verified.
