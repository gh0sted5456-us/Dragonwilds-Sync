(() => {
  ...
  function fixRouter(root=document){root.querySelectorAll('[data-router-home],a[href*="unifi.ui.com"],button[data-open-external*="unifi.ui.com"]').forEach(node=>{if(node.textContent!=='Open Default Router Homepage')node.textContent='Open Default Router Homepage';node.removeAttribute('data-open-external');node.removeAttribute('href');if(node.dataset.rc2Router!=='1'){node.dataset.rc2Router='1';node.addEventListener('click',openRouter,true)}})}
  function combineServerManagement(root=document){const settings=root.querySelector('[data-webhost-tab="settings"]');if(settings&&settings.textContent!=='Website & Directory')settings.textContent='Website & Directory';root.querySelectorAll('[data-webhost-tab="remote"]').forEach(b=>{if(b.textContent!=='Server Management')b.textContent='Server Management';b.classList.remove('rc2-retired')})}
