(() => {
  ...
  const defaultConsoleColors={game:'#79b8ff',ue4ss:'#f38b8b',runeschema:'#65d8cf',server:'#f1bf62',sync:'#72d39c'};
  ...
  function quickDragonLink() {
    ...
    const dragonLinkPanel=`<section class="v3q-panel v3q-dragonlink"><div class="v3q-panel-head"><div><b>DragonLink Application Bridge</b><small>Connect foundation; gameplay mods remain ordinary UE4SS packages</small></div><div class="v3q-item-status"><span class="v3q-live-chip ${installed?'ok':'bad'}"><i></i>${installed?(current?'INSTALLED · CURRENT':'INSTALLED · REPAIR AVAILABLE'):'NOT INSTALLED'}</span><button class="v3q-btn primary" data-v3q-dragonlink-save>Save &amp; Apply</button></div></div>${status.error?`<div class="v3q-error">${esc(status.error)}</div>`:''}<div class="v3q-dragonlink-modules">${moduleCard('connect','Connect','DragonLink-Connect.dll','Writes Direct Connect fields once when the game panel opens.')}</div><div class="v3q-dragonlink-settings">${toggle('enabled','Enable DragonLink','Master switch for the application bridge.')}${toggle('connect','Direct Connect handoff','Offer one-time address/password autofill to verified clients.')}</div><div class="v3q-dragonlink-foot"><code>${esc(component.path||'UE4SS/Mods/DragonLink')}</code><span>${locked?'Feature DLL toggles apply after stop.':'Changes apply on the next server start.'}</span></div></section>`;
    return dragonLinkPanel;
  }
