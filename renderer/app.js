(() => {
  ...
  function renderRsdwToolkit() {
    ...
    return `<div class="content rsdw-toolkit-page studio-combined-page">
      ...
      ${selector}
      ${characterTabs}
      <section class="studio-summary-card studio-combat-card studio-combat-first">${archetypeEditor}</section>
      ...
      <div class="rsdw-tool-launch-hint">Identity and progression now share one workspace. Appearance changes are available in Dragonwilds itself; choose an RSDW editor below for save-backed data.</div>${toolNav}
      ${editSurface}
  ...
  function renderHelp() {
    ...
    let guides = {
      ...
      characters: {
        title:'Characters and RSDW Editors', intro:'Select a character once to use combat identity, progression, inventory, spells, recipes, quests, and guarded save controls in one workspace.', image:'36-themed-item-editor.png', alt:'V1.1.9 native Item Editor using the launcher light theme', image2:'33-magic-wheel.jpg', alt2:'Formatted eight-slot Magic Wheel', image3:'34-recipe-categories.jpg', alt3:'Categorized RSDW recipes',
        sections:[
          ['One character workspace','Combat Identity appears first, followed by the selected character summary and World associations, then Character, Item, Spell, Recipe, and Quest editors. Identity & Appearance and Progression are no longer separate page modes. The embedded 3D viewer was removed; appearance can be changed in Dragonwilds without loading a second rendering application.','36-themed-item-editor.png','Current unified character workspace and Item Editor'],
      ...
      mods: {
        ...
        sections:[
          ...
          ['Identify the modder','Each managed directory mod receives identity.txt beside tags.txt and hotload.txt. Fill in Modder and Nexus so the author badge and safe Nexus link travel with the mod. Existing uppercase IDENTITY.txt files remain supported.'],
  ...
  function renderSettings() {
    ...
    if (tab === 'mods') {
      ...
      content = `
        ...
        <section class="settings-section"><div class="panel-header settings-section-head"><div><h2>RSDW Data &amp; Icons</h2><span class="panel-subtitle">Editor catalogs and the complete upstream icon manifest update independently of Dragonwilds Sync. Custom item icon bindings are preserved.</span></div><button class="btn primary" id="refresh-rsdw-cache">Check RSDW Now</button></div>
          <div class="module-status-grid"><div><span>RSDWTools</span><strong>${rsdw.toolkit_valid?'READY':'REFRESH NEEDED'}</strong><small>${escapeHtml(rsdw.revision?`Revision ${String(rsdw.revision).slice(0,12)} · ${rsdw.data_file_count||0} data · ${rsdw.icon_manifest_count||rsdw.icon_count||0} manifested icons`:'No validated local revision')}</small></div></div>
          <div class="settings-row"><div class="settings-copy"><strong>Automatic RSDW updates</strong><span>Checks RSDWTools in the background. A validated refresh atomically replaces upstream icon records while custom items keep their selected or embedded icons.</span></div><div class="settings-control-cluster"><select class="field compact-select" id="rsdw-refresh-hours"><option value="6" ${(a.rsdw_cache||{}).refresh_hours==6?'selected':''}>Every 6 hours</option><option value="12" ${(a.rsdw_cache||{}).refresh_hours==12?'selected':''}>Every 12 hours</option><option value="24" ${![6,12,48].includes(Number((a.rsdw_cache||{}).refresh_hours||24))?'selected':''}>Daily</option><option value="48" ${(a.rsdw_cache||{}).refresh_hours==48?'selected':''}>Every 2 days</option></select><button class="toggle ${(a.rsdw_cache || {}).auto_refresh === false ? '' : 'on'}" id="toggle-rsdw-auto-refresh"></button></div></div>
...
`;
  ...
  async function openModExplorer(scope, unitKey) {
    ...
    try {
      ...
      const groupFor=(file)=>{const path=String(file.relative_path||'').replace(/\\/g,'/'),base=path.split('/').pop().toLowerCase(),ext=base.split('.').pop();if(base==='hotload.txt'||base==='tags.txt'||base==='identity.txt')return 'Sync Identifiers';if(['pak','ucas','utoc'].includes(ext))return '/Paks';const dir=path.includes('/')?path.slice(0,path.lastIndexOf('/')):'';return dir?`/${dir}`:`/${name}`;};
  ...
  function openServerEditor(world) {
    ...
    showModal(`<div class="modal-header"><div><h2>Edit Hosted World</h2><p>Presentation, credentials, ports, health evidence, access policy, and World runtime preferences. The shared server installation lives in Settings → Server.</p></div><button class="modal-close" data-close-modal>×</button></div><div class="modal-body">
      <div class="form-grid">
        ...
        <div class="form-group"><label>UE4SS mods.txt</label><input type="hidden" id="se-mods-txt-mode" value="auto"/><span class="status-pill online">AUTOMATIC · HIDDEN</span><small>Dragonwilds Sync writes one enabled entry per selected UE4SS mod in the exact <code>MODNAME : 1</code> format. RuneSchema self-enables separately.</small></div>
