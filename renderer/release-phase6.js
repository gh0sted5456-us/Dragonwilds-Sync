  ...
  async function hydrate(host, token) {
    try {
      ...
      if (summary) summary.innerHTML = `
        <div><strong>DragonLink-Connect</strong><span>${esc(dc.status || (dc.installed ? 'installed' : 'not installed'))}${dc.available_version ? ` · ${esc(dc.available_version)}` : ''}</span></div>
  ...
  async function openCommunity(button) {
    ...
    try {
      ...
      content.innerHTML = `<div class="phase6-community-page">
        ...
        <section class="settings-section"><h2>Final Integration State</h2><div class="phase6-integration-summary" data-phase6-integration-summary><div><strong>DragonLink-Connect</strong><span>Checking local managed state…</span></div><div><strong>Last verified Sync</strong><span>Checking journal…</span></div><div><strong>Credential storage</strong><span>Checking local reference vault…</span></div></div><div class="identity-box"><strong>Source ownership</strong><p><b>RSDWTools</b> supplies icons/item/reference data. <b>RSDW Toolkit / DevKit</b> is the UE4SS runtime tooling mod. <b>DragonLink-Connect</b> is the hidden client/host handoff component; <code>DragonConnectHelper</code> and <code>PersistentDirectConnectIP</code> remain migration aliases only.</p></div></section>
