...
- Packaged metadata is 3.5.1 on `main`; tagged release builds stamp their tag version before packaging so update comparisons use the version actually being distributed. `testing-branch` is reserved for staged verification and package candidates.
