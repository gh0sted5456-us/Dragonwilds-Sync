(() => {
  ...
  function renderUnifiedConsole(payload) {
    ...
    host.innerHTML = `<div class="panel-header dws-unified-console-head"><div><h2>Unified Console</h2><span class="panel-subtitle">Game CMD/stdout, UE4SS, dedicated-server events, and World Sync traffic in one stream.</span></div><div class="dws-console-controls"><button class="btn ghost compact-btn" data-dws-console-pause>${consolePaused ? 'Resume' : 'Pause'}</button></div></div>
      <div class="dws-console-filters"><button class="${consoleFilter === 'all' ? 'active' : ''}" data-dws-console-filter="all">ALL <b>${number((counts.game || 0) + (counts.ue4ss || 0) + (counts.server || 0) + (counts.sync || 0), '0')}</b></button><button class="${consoleFilter === 'game' ? 'active' : ''}" data-dws-console-filter="game">GAME <b>${number(counts.game || 0, '0')}</b></button><button class="${consoleFilter === 'ue4ss' ? 'active' : ''}" data-dws-console-filter="ue4ss">UE4SS <b>${number(counts.ue4ss || 0, '0')}</b></button><button class="${consoleFilter === 'server' ? 'active' : ''}" data-dws-console-filter="server">SERVER <b>${number(counts.server || 0, '0')}</b></button><button class="${consoleFilter === 'sync' ? 'active' : ''}" data-dws-console-filter="sync">SYNC <b>${number(counts.sync || 0, '0')}</b></button></div>
